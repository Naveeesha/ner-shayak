...
-- -----------------------------------------------------------------------------
-- 13. NOTIFICATION LOGS (SMS & System Delivery History)
-- -----------------------------------------------------------------------------
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_notification_logs_idempotency ON notification_logs(event_type, event_id, recipient_phone) WHERE status IN ('queued', 'sent');
...
-- RLS policies on storage.objects for incident-photos bucket
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'objects' AND schemaname = 'storage' AND policyname = 'Public Access Incident Photos'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'objects' AND schemaname = 'storage' AND policyname = 'Allow Uploads Incident Photos'
  ) THEN
    CREATE POLICY "Allow Uploads Incident Photos"
      ON storage.objects FOR INSERT
      WITH CHECK (bucket_id = 'incident-photos');
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'objects' AND schemaname = 'storage' AND policyname = 'Allow Updates Incident Photos'
  ) THEN
    CREATE POLICY "Allow Updates Incident Photos"
